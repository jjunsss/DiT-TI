...
class CUB200Dataset(torch.utils.data.Dataset):
    def __init__(self, img_dir, images_file, labels_file, split_file, classes_file, json_file_train, json_file_test, transform=None, train=True):
        self.img_dir = img_dir
        self.transform = transform
        self.train = train

        # Load image paths and image names
        self.image_paths = {}
        self.image_names = {}
        self.image_ids = {}
        with open(images_file, 'r') as f:
            for line in f:
                ...
                self.image_paths[image_id] = os.path.join(img_dir, image_name)
                self.image_names[image_id] = image_name
                self.image_ids[os.path.join(img_dir, image_name)] = image_id
        ...
        with open(labels_file, 'r') as f:
            for line in f:
                ...
                self.image_labels[int(image_id)] = int(class_id) - 1  # Class labels are 1-indexed
        ...
        with open(split_file, 'r') as f:
            for line in f:
                ...
                if int(is_training) == (1 if train else 0):
                    self.split.append(int(image_id))
        ...
        # Load descriptions based on training mode
        self.image_descriptions = {}
        json_file = json_file_train if train else json_file_test
        with open(json_file, 'r') as f:
            data = json.load(f)
            for item in data:
                image_path = item['image_path']
                # Get image_name relative to img_dir
                image_name = os.path.basename(image_path)
                self.image_descriptions[image_name] = item['description']

    def __len__(self):
        return len(self.split)

    def __getitem__(self, idx):
        image_id = self.split[idx]
        image_path = self.image_paths[image_id]
        image = Image.open(image_path).convert('RGB')
        label = self.image_labels[image_id]
        class_name = self.class_names[label].split('.')[1]

        image_name = self.image_names[image_id]
        description = self.image_descriptions.get(os.path.basename(image_name), '')

        if self.transform:
            image = self.transform(image)

        temp_features = torch.zeros((384, 64, 64)).contiguous()
        return image, label, image_path, class_name, description, temp_features