...
def save_model_card(
    ...
    if images is not None:
        for i, image in enumerate(images):
            ...
            widget_dict.append({"text": validation_prompt if validation_prompt else " ", "output": {"url": f"image_{i}.png"}})
    diffusers_load_lora = ""
    ...
    if train_text_encoder_ti:
        ...
        ti_keys = ", ".join(f'"{match}"' for match in re.findall(r"<[^>]+>", instance_prompt))  # ! edited
...
def log_validation(
    ...
    with torch.no_grad():
        prompt_embeds, pooled_prompt_embeds, text_ids = pipeline.encode_prompt(pipeline_args["prompt"], prompt_2=pipeline_args["prompt"])
    images = []
    for _ in range(args.num_validation_images):
        with autocast_ctx:
            image = pipeline(prompt_embeds=prompt_embeds, pooled_prompt_embeds=pooled_prompt_embeds, generator=generator).images[0]
            images.append(image)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="number of new tokens inserted to the tokenizers per token_abstraction identifier when " "--train_text_encoder_ti = True. By default, each --token_abstraction (e.g. TOK) is mapped to 2 new " "tokens - <si><si+1> ",
    )
    parser.add_argument(
        "--initializer_concept",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help=("Run dreambooth validation every X epochs. Dreambooth validation consists of running the prompt" " `args.validation_prompt` multiple times: `args.num_validation_images`."),
    )
    parser.add_argument(
        "--rank",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="coefficients for computing the Prodigy stepsize using running averages. If set to None, " "uses the value of square root of beta2. Ignored if optimizer is adamW",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=("Whether to upcast the trained transformer layers to float32 before saving (at the end of training). " "Defaults to precision dtype used for training to save memory"),
    )
    parser.add_argument(
    ...
    if args.train_text_encoder and args.train_text_encoder_ti:
        raise ValueError("Specify only one of `--train_text_encoder` or `--train_text_encoder_ti. " "For full LoRA text encoder training check --train_text_encoder, for textual " "inversion training check `--train_text_encoder_ti`")
    if args.train_transformer_frac < 1 and not args.train_text_encoder_ti:
        raise ValueError("--train_transformer_frac must be == 1 if text_encoder training / textual inversion is not enabled.")
    ...
    if args.train_text_encoder_ti and args.initializer_concept and args.num_new_tokens_per_abstraction:
        logger.warning("When specifying --initializer_concept, the number of tokens per abstraction is detrimned " "by the initializer token. --num_new_tokens_per_abstraction will be ignored")
...
class TokenEmbeddingsHandler:
    ...
    def initialize_new_tokens(self, inserting_toks: List[str]):
        ...
        for tokenizer, text_encoder in zip(self.tokenizers, self.text_encoders):
            ...
            if args.initializer_concept is None:
                ...
                embeds.weight.data[train_ids] = torch.randn(len(train_ids), hidden_size).to(device=self.device).to(dtype=self.dtype) * std_token_embedding
            else:
    ...
    def retract_embeddings(self):
        for idx, text_encoder in enumerate(self.text_encoders):
            ...
            embeds.weight.data[index_no_updates] = self.embeddings_settings[f"original_embeddings_{idx}"][index_no_updates].to(device=text_encoder.device).to(dtype=text_encoder.dtype)
        ...
        if args.dataset_name is not None:
            ...
            if args.image_column is None:
                ...
                if image_column not in column_names:
                    raise ValueError(f"`--image_column` value '{args.image_column}' not found in dataset columns. Dataset columns are: {', '.join(column_names)}")
            instance_images = dataset["train"][image_column]

            if args.caption_column is None:
                logger.info("No caption column provided, defaulting to instance_prompt for all images. If your dataset " "contains captions/prompts for the images, make sure to specify the " "column as --caption_column")
                self.custom_instance_prompts = None
            else:
                if args.caption_column not in column_names:
                    raise ValueError(f"`--caption_column` value '{args.caption_column}' not found in dataset columns. Dataset columns are: {', '.join(column_names)}")
...
def main(args):
    if args.report_to == "wandb" and args.hub_token is not None:
        raise ValueError("You cannot use both --report_to=wandb and --hub_token due to a security risk of exposing your token." " Please use `hf auth login` to authenticate with the Hub.")
    ...
    if args.train_text_encoder_ti:
        ...
        # if args.initializer_concept is provided, we ignore args.num_new_tokens_per_abstraction
            ...
            logger.info(f"initializer_concept: {args.initializer_concept}, num_new_tokens_per_abstraction: {num_new_tokens_per_abstraction}")

        # ! custom init
        class_name = args.dataset_name.split("/")[-2].split(".")[-1]
        class_token = f"<{class_name}>"
        logger.info(f"[FOR CHECKING] class token is here: {class_name}")

        token_abstraction_dict = {}
        for token in token_abstraction_list:
            tokens_for_this_abstraction = []
            if num_new_tokens_per_abstraction == 1:
                tokens_for_this_abstraction.append(class_token)
            else:
                # multi-token TI를 쓰고 싶다면 <Class_0>, <Class_1> ... 이런 식으로 확장
                for j in range(num_new_tokens_per_abstraction):
                    tokens_for_this_abstraction.append(f"{class_token[:-1]}_{j}>")  # "<Class>"" -> "<Class_0>"

            token_abstraction_dict[token] = tokens_for_this_abstraction

        # 5) 프롬프트 안의 TOK 를 "<Class...>" 로 치환
        for token_abs, token_replacement in token_abstraction_dict.items():
            replacement_str = "".join(token_replacement)
            new_instance_prompt = args.instance_prompt.replace(token_abs, replacement_str)
            if args.instance_prompt == new_instance_prompt:
                logger.warning(
                    "Note! the instance prompt provided in --instance_prompt does not include the token abstraction " "specified in --token_abstraction. This may lead to incorrect optimization of text embeddings " "during pivotal tuning"
                )
            args.instance_prompt = new_instance_prompt
            if args.with_prior_preservation and args.class_prompt is not None:
                args.class_prompt = args.class_prompt.replace(token_abs, replacement_str)
            if args.validation_prompt:
                args.validation_prompt = args.validation_prompt.replace(token_abs, replacement_str)
        # ! custom init

        logger.info(f"[FOR CHECKING] instance_prompt is here: {args.instance_prompt}")
        logger.info(f"[FOR CHECKING] token_abstraction_dict is here: {token_abstraction_dict}")
    ...
    if args.use_8bit_adam and not args.optimizer.lower() == "adamw":
        logger.warning(f"use_8bit_adam is ignored when optimizer is not set to 'AdamW'. Optimizer was " f"set to {args.optimizer.lower()}")
    ...
    if args.with_prior_preservation:
        if freeze_text_encoder:
            class_prompt_hidden_states, class_pooled_prompt_embeds, class_text_ids = compute_text_embeddings(args.class_prompt, text_encoders, tokenizers)
    ...
    if not train_dataset.custom_instance_prompts:
        ...
        # we need to tokenize and encode the batch prompts on all training steps
            ...
            tokens_one = tokenize_prompt(tokenizer_one, args.instance_prompt, max_sequence_length=77, add_special_tokens=add_special_tokens_clip)
    ...
    if args.max_train_steps is None:
        ...
        num_training_steps_for_scheduler = args.num_train_epochs * accelerator.num_processes * num_update_steps_per_epoch
    else:
    ...
    if not freeze_text_encoder:
        if args.enable_t5_ti:
            ...
            transformer, text_encoder_one, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(transformer, text_encoder_one, optimizer, train_dataloader, lr_scheduler)
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if train_dataset.custom_instance_prompts:
                    ...
                    if freeze_text_encoder:
                        ...
                        tokens_one = tokenize_prompt(tokenizer_one, prompts, max_sequence_length=77, add_special_tokens=add_special_tokens_clip)
                        tokens_two = tokenize_prompt(
                            tokenizer_two,
                            prompts,
                            max_sequence_length=args.max_sequence_length,
                ...
                if args.with_prior_preservation:
                    ...
                    prior_loss = torch.mean(
                        (weighting.float() * (model_pred_prior.float() - target_prior.float()) ** 2).reshape(target_prior.shape[0], -1),
                        1,
                    )
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        if args.train_text_encoder_ti:
                            embedding_handler.save_embeddings(f"{args.output_dir}/{Path(args.output_dir).name}_emb_checkpoint_{global_step}.safetensors")
